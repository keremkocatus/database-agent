...
| 14 | [Güvenlik ve Gizlilik](14-security-and-privacy.md) | Provider seçimi (redaction yok), per-user görünürlük, prompt-injection guardrail, serving erişim/kapsam |
...
| 19 | [Deployment ve Operasyon](19-deployment-and-operations.md) | docker-compose, init/doctor, bootstrap, yedek/restore (DR), token-tabanlı kill-switch, quickstart |
| 20 | [Eşzamanlılık, Kapasite ve Performans](20-concurrency-capacity-and-performance.md) | Havuz-tabanlı paralellik, AI instance yönetimi, token yönetimi, çok-kullanıcı + kapasite sinyali, perf-test |
| — | [İnceleme: Boşluk Analizi](REVIEW-gap-analysis.md) | Tasarım gözden geçirmesi + bu turda alınan kararların gerekçesi |
...
- Güvenlik: **dışlama (exclusion)** ile kritik tablo/SP tamamen görünmez; **per-user rol-bazlı görünürlük** (`deny`); prompt-injection guardrail; serving scope auth. **Redaction YOK** → hassas içerik için `allow_cloud`/provider seçimi kullanıcı sorumluluğunda (`14`).
- Observability: **Prometheus+Grafana+OTel+JSON log + token-merkezli ölçüm + kaynak kök-neden** (`16`); Chatbot: **kalıcı oturum + pencere/özet/semantik bellek** (`17`); Etkileşim: **netleştirme + onay/feedback + tipli SSE streaming** (`18`).
- Eşzamanlılık: **havuz-tabanlı paralellik + kuyruk** (sistem kilitlenmez), AI instance yönetimi, **token yönetimi**, çok-kullanıcı + **kapasite sinyali**, kullanıcı-tetikli **perf-test** (`20`).
- Kalite güvenceleri: **enrichment kalite kapısı** (zehirli embedding önleme, `05`/`06`), **re-embed sırasında eski-set servis** (`07`), **fail-listesi** (2 retry→ayrı, aramaya katılmaz, `09`), **süreç sürüm uyumu** (payload version + beklet, `11`).
- Otorite: disk=içerik; Postgres indeksi atılabilir ama **feedback/chat/runs otoriter → yedeklenir**; **ham SQL disk-only (kritik yedek)**; DR + bootstrap + docker-compose + init/doctor (`19`). Açık-kaynak default modeller **Apache-lisanslı** (Qwen/BGE).

## Durum
Tasarım fazı tamamlandı (00–20 elden geçti; ikinci tur kararlar `REVIEW-gap-analysis.md`'deki bulgulara göre işlendi). Kalan açık sorular `13` sonunda (GPU spesifikasyonu, altın-set sahibi, gerçek kapasite → `perf-test`). Sonraki adım: M0 iskeletinin kurulması.
